"""
这个程序模拟了一个最简单的 人工智能学习过程 ：

1. 给数据 （题目和答案）。
2. 建模型 （造一个能计算的大脑）。
3. 算误差 （看看猜得对不对）。
4. 改参数 （根据误差调整大脑，让它下次猜得更准）。
"""
...
plt.figure(figsize=(8, 5))
plt.plot(range(1, batch_size + 1), y_actual,
         'o-', label='Actual', color='blue')
plt.plot(range(1, batch_size + 1), y_pred_final,
         'x--', label='Predicted', color='red')
